 ...
 */
export interface PcmRouterEventBridgeOptions {
  /** Unix socket path. */
  socketPath: string;
  /** Structured logger. */
  logger: Logger;
...
/**
 * Subscribes to native PCM router event envelopes and forwards them to SSE clients.
 */
export class PcmRouterEventBridge {
  ...
  private connect(): void {
    ...
    socket.on("connect", () => {
      ...
      this.options.logger.info({
        ...
        correlationId: "pcm-event-bridge",
        message: "PCM router event bridge connected",
      });
      socket.write('{"method":"subscribe","correlationId":"pcm-event-bridge"}\n');
    });
