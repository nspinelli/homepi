...
/**
 * Options for the USB devices SSE event bridge.
 */
export interface UsbDevicesEventBridgeOptions {
  /** Unix socket path. */
  socketPath: string;
  /** Structured logger. */
  logger: Logger;
...
/**
 * Subscribes to native USB devices event envelopes and updates system status.
 */
export class UsbDevicesEventBridge {
   ...
   */
  constructor(private readonly options: UsbDevicesEventBridgeOptions) {
    this.reconnect = new EventBridgeReconnect({
      logger: options.logger,
      module: "app.backend.usb-devices",
      correlationId: "usb-event-bridge",
  ...
  private connect(): void {
    ...
    socket.on("connect", () => {
      ...
      this.options.logger.info({
        ...
        correlationId: "usb-event-bridge",
        message: "USB devices event bridge connected",
      });
      socket.write('{"method":"subscribe","correlationId":"usb-event-bridge"}\n');
    });
    ...
    socket.on("close", () => {
      ...
      this.setConnected(false);
      if (!this.stopped) {
        this.options.coordinator.markServiceOffline("usbDevices", "usb-event-bridge");
