#!/usr/bin/env bash
# Uninstalls homepi-usb-devices systemd service and /opt install tree.
set -euo pipefail
...
SOCKET_PATH="/run/homepi/usb-devices.sock"
...
remove_socket() {
  if [[ -S "${SOCKET_PATH}" ]]; then
    log "Removing socket ${SOCKET_PATH}"
    rm -f "${SOCKET_PATH}"
  fi
}
...
main() {
  ...
  echo "  SQLite DB and generated artifacts under /opt/homepi/runtime were kept."
  echo "  Remove manually if needed: /opt/homepi/runtime/state/homepi.sqlite"
}
